...
/*   Updated: 2018/05/15 19:14:31 by iseletsk         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	s_helpa_codg(t_proc *proc, t_con *con)
{
	...
	if ((proc->mem)[proc->index]->byte == 16)
		do_aff(con, proc->index, codg, proc);
}
...
void		vm_hendl_command(t_proc *proc, t_con *con)
{
			...
			vm_give_codg(proc, codg))
	{
		...
		if ((proc->mem)[proc->index]->byte == 4)
			do_add(con, proc->index, codg, proc);
		if ((proc->mem)[proc->index]->byte == 5)
			do_sub(con, proc->index, codg, proc);
